...
QLTHUMBNAILS_ENTITLEMENTS=$(dirname "$0")/SkimQuickLookThumbnails/SkimQuickLookThumbnails.entitlements
QLPREVIEW_ENTITLEMENTS=$(dirname "$0")/SkimQuickLookPreview/SkimQuickLookPreview.entitlements
...
codesign ${CODESIGN_FLAGS} ${CODESIGN_OPTIONS} -s "${IDENTITY}" "${LOCATION}/SkimTransitions.plugin/Contents/MacOS/SkimTransitions"
codesign ${CODESIGN_FLAGS} ${CODESIGN_OPTIONS} -s "${IDENTITY}" --entitlements "${QLTHUMBNAILS_ENTITLEMENTS}" "${LOCATION}/SkimQuickLookThumbnails.appex/Contents/MacOS/SkimQuickLookThumbnails"
codesign ${CODESIGN_FLAGS} ${CODESIGN_OPTIONS} -s "${IDENTITY}" --entitlements "${QLPREVIEW_ENTITLEMENTS}" "${LOCATION}/SkimQuickLookPreview.appex/Contents/MacOS/SkimQuickLookPreview"
